...
def parse_data(input_data: list[str]) -> list[list[int]]:
    ...
    for data in input_data:
        *_, props = data.split(": ")
        ...
        properties.append(list(map(int, props)))  # convert numbers to int
    return list(map(list, zip(*properties)))  # gather by properties


def compute_score(input_data: list[list[int]],
                  teaspoon_weights: list[int],
                  activation: list[int]):
    scores = [sum([a*b for a, b in zip(prop, teaspoon_weights)])
              for prop in input_data]
    return prod([score * active for score, active in zip(scores, activation)
                if active * score > 0])
